...
public class AppGUI implements AppStyleArbiter {
    ...
    private void initFileToolbar(AppTemplate app) {
        ...
        String imagePath = FILE_PROTOCOL + PATH_IMAGES + "Photo.png";
        Image buttonImage = new Image(imagePath);
        photoButton.setGraphic(new ImageView(buttonImage));
        Tooltip buttonTooltip = new Tooltip("Save this design as an image");
        photoButton.setTooltip(buttonTooltip);
        photoButton.setDisable(true);
        codeButton = new Button();
        imagePath = FILE_PROTOCOL + PATH_IMAGES + "Code.png";
        buttonImage = new Image(imagePath);
        codeButton.setGraphic(new ImageView(buttonImage));
        buttonTooltip = new Tooltip("Export as Java code");
        codeButton.setTooltip(buttonTooltip);
        codeButton.setDisable(true);
        exports = new VBox();
        exports.getChildren().addAll(photoButton,codeButton);
        fileToolbarPane.getChildren().add(exports);
        exitButton = initChildButton(fileToolbarPane,	EXIT_ICON.toString(),	    EXIT_TOOLTIP.toString(),	false);
        fileToolbarPane.setPrefWidth(325);
        fileToolbarPane.setHgap(2);
	...
	exitButton.getStyleClass().add(CLASS_FILE_BUTTON);
        topPane.getStyleClass().add(CLASS_TOP_PANE);
    }
    
    public void addToTopPane(FlowPane Toolbar){
        if(topPane.getChildren().size()<3)topPane.getChildren().add(Toolbar);
    }
}
